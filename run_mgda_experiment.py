...
def main():
    print("\n==========================================")
    print("BIDCell MGDA Experiment")
    print("Features: Memory Check, Full Metrics (Pos/Neg ExprsPct), Safe Save")
    print("==========================================\n")
    
    # 1. Setup Directories
    base_dir = os.path.dirname(os.path.abspath(__file__))
    mgda_results_dir = os.path.join(base_dir, "mgda_results")
    if not os.path.exists(mgda_results_dir):
        os.makedirs(mgda_results_dir)
    ...
    for lr in lrs:
        ...
        # Reduced memory check for small dataset
        if not check_memory(required_gb=8.0):
            print("  Memory low. Waiting 30s to drain...")
            time.sleep(30)
            if not check_memory(required_gb=6.0):
        ...
        try:
            ...
            # Force aggregation to mgda just in case it wasn't picked up
            model.config.training_params.aggregation = "mgda"
