...
def plot_loss_curves(training_history, out_dir):
    ...
    plt.xlabel('Step')
    plt.ylabel('Loss')
    plt.title('Training Loss Curves (CAGrad)')
    plt.legend()
    plt.grid(True)
...
def main():
    cagrad_results_dir = os.path.join("D:\\2512-BROCK-CODING\\BIDCell", "cagrad_results")
    if not os.path.exists(cagrad_results_dir):
    ...
    # Set Aggregation to CAGrad
    print("Using CAGrad Aggregation...")
    model.config.training_params.aggregation = "cagrad"
    ...
    with open(os.path.join(cagrad_results_dir, "training_metrics.json"), "w") as f:
        json.dump({"history": training_points, "learning_rates": lrs}, f, indent=4)
        
    plot_loss_curves(training_points, cagrad_results_dir)
    ...
    if connected_files:
        ...
        with open(os.path.join(cagrad_results_dir, "morphology_metrics.json"), "w") as f:
    ...
    if cgm_files:
        ...
        with open(os.path.join(cagrad_results_dir, "expression_metrics.json"), "w") as f:
    ...
    print(f"Results saved to {cagrad_results_dir}")
