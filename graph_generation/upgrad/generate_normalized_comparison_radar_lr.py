...
def generate_normalized_comparison_radar_lr():
    """
    ...
    legend_map = {
        ...
        # 'ugrad' will be handled dynamically
    }
    ...
    for ugrad_key, ugrad_selected_data in ugrad_data.items():
        ...
        # Define method order for this plot
        # We use the keys from legend_map for the CSV-based ones, but we need to use the RENAMED keys for the plot
        # Actually, let's normalize first, then rename for the dataframe
        ...
        for method_internal in methods_to_plot_internal:
            ...
            if method_internal in legend_map:
                ...
                method_display = method_internal # e.g. UPGrad
            
            normalized_data[method_display] = []
            
            # Fetch data source (handle mapped names vs internal names)
            # current_methods_data uses 'default', 'amtl-min' etc.
        ...
        fig.update_layout(
            ...
            polar=dict(
                radialaxis=dict(
                    visible=True,
                    range=[0, 2],
                    tickfont=dict(size=24),  # Increased from 18
                ),
                angularaxis=dict(
                    tickfont=dict(size=28),  # Increased from 18 to 28 for better visibility
                )
            ),
            legend=dict(
                title=dict(text="method", font=dict(size=22)), # Increased title size
                font=dict(size=22) # Increased from 16
            ),
            font=dict(size=20) # Increased from 14
        )
        ...
        output_png = os.path.join(output_dir, f"normalized_comparison_radar_{clean_key}.png")
...
if __name__ == "__main__":
    generate_normalized_comparison_radar_lr()
