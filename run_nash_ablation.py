...
def main():
    ...
    out_dir = "nashmtl_ablation_results"
    ...
    # Isolate Data
    data_dir = "nashmtl_ablation_data"
    ...
    for name, params in ablations.items():
        print(f"\n=== Running Ablation (Nash-MTL): {name} ===")
        
        # Modify Config
        config = copy.deepcopy(base_config)
        config['training_params']['aggregation'] = 'nashmtl'
...
def plot_radar(data, out_dir):
    ...
    labels = [k.replace("area", "cell_area") + "*" for k in plot_keys]
    num_vars = len(labels)
